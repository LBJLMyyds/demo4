...
(() => {
  ...
  function initQualityView() {
    ...
    stats.innerHTML = [
      { n: c.conflicts, l: "Open conflicts" },
    ...
    $("#issue-cards").innerHTML = `
      <div class="issue-card">
        ...
        <p>Rows whose coordinates fall outside the Victoria bounding box — mostly venues in other Australian
          states or overseas, likely picked up when matching venue names to Google Places IDs upstream. Excluded
          from the map rather than shown at the wrong location.</p>
      ...
      <div class="issue-card">
        <h3>${svg("check")} Conflict detection</h3>
        <div class="issue-num">${state.meta.counts.conflicts}</div>
        <p>Open disagreements between community reports on the same venue + feature, out of
          ${state.meta.counts.places.toLocaleString()} mapped venues — surfaced here instead of silently
          averaged away.</p>
      </div>`;
  }
  ...
  function renderQualityTable() {
    ...
    if (!rows.length) {
      tbody.innerHTML = `<tr class="qt-empty"><td colspan="4">No flagged venues match this filter.</td></tr>`;
      return;
    }
    tbody.innerHTML = rows.map(c => `
      <tr>
        ...
        <td><span class="qt-status">${escapeHtml(c.status)}</span></td>
      </tr>`).join("");
  }
